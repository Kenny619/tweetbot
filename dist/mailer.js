...
const sendMail = {
    belowTh: () => {
        transporter.sendMail(belowThOption, (e, reply) => {
            if (e)
                Log.error("Email", `Failed to send 'Below Threshold' email.  ${e}`);
            if (reply)
                Log.info("Email", "Successfully sent 'Below Threshold' email.");
    ...
    noImages: () => {
        transporter.sendMail(noMoreImages, (e, reply) => {
            if (e)
                Log.error("Email", `Failed to send 'No images' email. ${e}`);
            if (reply)
                Log.info("Email", "Successfully sent 'No images' email.");
    ...
    postFialed: () => {
        transporter.sendMail(postFialed, (e, reply) => {
            if (e)
                Log.error("Email", `Failed to send 'Post failed' email.  ${e}`);
            if (reply)
                Log.info("Email", "Successfully sent 'No images' email.");
